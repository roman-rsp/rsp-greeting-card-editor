 ...
 * Wir haben hier nun zwei unterschiedliche "Seiten" in den Mock-Daten,
 * um den Wechsel visuell sofort prüfen zu können.
 */
const MOCK_DATA = {
  "wk_motiv": {
    "project": {
      ...
      "objects": {
        ...
        "Block_302_p0": {
          ...
          "layer": "bearbeitung", "content": "VORDERSEITE (Master)", "fontSize": 32, "fontFamily": "Poppins", "zIndex": 999
        },
        "Block_293_p0": {
    ...
    "project": {
      ...
      "objects": {
        "Block_451_p0": {
          ...
          "layer": "bearbeitung", "content": "INNENSEITE (Layout 1)",
          "fontSize": 24, "fontFamily": "Poppins Light", "zIndex": 999
        },
        "Block_322_p0": {
...
const App = () => {
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50">
      ...
      <p className="text-[10px] uppercase font-bold tracking-[0.3em] text-slate-400 font-sans">Synchronisiere Ebenen...</p>
    </div>
  );

  // --- LOGIK FÜR DIE SEITENWAHL ---
  ...
  // FIX: Wenn "inside" aktiv ist und das Dokument eine "page_1" hat (2-Seiter), nutze diese.
  // Ansonsten nutze "page_0" (für separate Master-Templates).
  const pageKey = (activeSide === 'inside' && currentData.pages['page_1']) ? 'page_1' : 'page_0';
  const currentPage = currentData.pages[pageKey];
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden text-left text-[14px]">
      ...
      <nav className="w-24 bg-slate-900 flex flex-col items-center py-8 gap-10 shadow-2xl z-[100]">
        <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center text-white font-bold italic text-xl border border-white/10 text-left">RSP</div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 z-20 shadow-sm">
          <div className="flex flex-col">
              <span className="text-[9px] uppercase font-bold text-indigo-600 tracking-[0.3em] mb-1 italic">RSP Editor v3.4.0</span>
      ...
      {/* Debug-Inspektor */}
      <aside className="w-96 bg-white border-l border-slate-200 flex flex-col shadow-2xl z-40 text-left">
        <div className="p-8 border-b border-slate-100 bg-slate-50/40 flex-1 overflow-y-auto custom-scrollbar">
            <h2 className="font-bold uppercase text-[10px] tracking-[0.4em] text-slate-400 mb-8 flex items-center gap-2"><Bug size={12} className="text-indigo-500 text-left" /> Röntgen-Liste ({pageKey})</h2>
            
            <div className="space-y-2 mb-8">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-left block">Objekte auf aktueller Seite:</span>
              {objects.map(o => (
                <div key={o.id} className={`p-2 rounded text-[10px] flex justify-between items-center ${selectedId === o.id ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-100 text-slate-600'}`}>
                  <span className="text-left">{o.type === 'text' ? '📝' : '🖼️'} {o.id}</span>
                  <span className="font-mono opacity-60">z:{o.type === 'text' ? 900 : 10}</span>
                </div>
              ))}
            </div>

            <h2 className="font-bold uppercase text-[10px] tracking-[0.4em] text-slate-400 mb-6 text-left">Bearbeitung</h2>
            
            {selectedElement && selectedElement.type === 'text' ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-5 text-left">
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm focus-within:border-indigo-500 transition-colors">
                        <textarea className="w-full text-sm outline-none border-none p-0 bg-transparent resize-none h-48 text-slate-700 leading-relaxed font-sans text-left"
